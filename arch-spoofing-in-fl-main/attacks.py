...
_LEARNABLE_MASK_CACHE: dict = {}


def _learnable_mask(arch, input_shape, num_classes, n) -> np.ndarray:
    """Boolean mask over the flat vector: True where a coordinate is a LEARNABLE
    parameter, False where it is a non-learnable buffer (BatchNorm statistics).

    The flat vector is `state_dict()` concatenated in SORTED KEY ORDER
    (`aggregation.flatten`), so the mask is built by walking those same sorted
    keys and asking whether each one is a registered parameter. Cached per
    architecture because it is rebuilt every round otherwise.

    Falls back to all-True when the architecture cannot be resolved, which keeps
    the single-architecture TensorFlow path working unchanged.
    """
    key = (arch, int(n))
    if key in _LEARNABLE_MASK_CACHE:
        return _LEARNABLE_MASK_CACHE[key]
    mask = np.ones(int(n), dtype=bool)
    try:
        from aggregation import sorted_keys
        from lab_probe import load_torch_weights

        model = load_torch_weights(arch, np.zeros(int(n)), input_shape, num_classes)
        state = model.state_dict()
        learnable = set(dict(model.named_parameters()).keys())
        mask = np.concatenate([
            np.full(int(state[k].numel()), k in learnable, dtype=bool)
            for k in sorted_keys(state)])
        if mask.size != int(n):            # template disagrees with the vector
            mask = np.ones(int(n), dtype=bool)
    except Exception:
        pass
    _LEARNABLE_MASK_CACHE[key] = mask
    return mask
...
class WeightBoost(Attack):
    ...
    There are two paths, because the two servers hand the attacker different
    things:

      TENSORFLOW (`apply`)  operates on `update.weights`, and needs the caller to
        set `global_weights` each round, since the delta is measured against
        whatever model the client warm-started from.
      TORCH (`shape_update`)  needs no such protocol. `TorchFederatedServer` has
        already written `metadata['delta_vec'] = vec - global_vec` by the time
        `shape_update` runs, so the warm-start point is recoverable as
        `vec - delta_vec` and the boost is exact.

    Use whichever matches the loop. `apply` is a no-op when `global_weights` was
    never set, which is what keeps the torch path from being boosted twice.
    """
    ...
    def shape_update(self, update, context: dict):
        """Torch path: boost the flat vector using the server's own delta.

        The delta the clusterer sees has to be re-derived, not left stale: the
        server computed it before this hook ran, and after boosting the update it
        actually receives is `boost` times further from the warm-start point.
        Note a cosine clusterer cannot see this at all -- scaling a vector does
        not change its direction -- which is exactly why the boost is a payload
        and not a placement mechanism.

        BATCHNORM BUFFERS ARE EXCLUDED, and that is not a detail. The flat vector
        is the whole `state_dict`, so it carries `running_mean`, `running_var` and
        `num_batches_tracked` alongside the learnable weights -- 1,924 of
        `dnn_flat`'s 578,894 coordinates. Scaling `running_var` drives it
        NEGATIVE, the forward pass takes a square root of it, and the model emits
        NaN. Measured: boosting the full vector by as little as 1.2x sends victim
        accuracy to 0.0000 with NaN logits, which reads as a devastating poison
        and is really just numerical divergence -- and divergence is the single
        cheapest thing for a server to detect. That would be the same trap the
        sign-flip result already documents. Model replacement (Bagdasaryan et al.)
        amplifies the LEARNED update, so the buffers are left alone.
        """
        delta = update.metadata.get("delta_vec")
        if update.client_id not in self.malicious_clients or delta is None:
            return update
        arch = update.metadata.get("arch") or context.get("arch")
        mask = _learnable_mask(arch, context.get("input_shape"),
                               context.get("num_classes"), len(update.vec))
        scaled = np.where(mask, self.boost * delta, delta)
        update.vec = (update.vec - delta) + scaled
        update.metadata["delta_vec"] = scaled
        return update
...
class ArchSpoof(Attack):
    ...
    def apply(self, update: ClientUpdate) -> ClientUpdate:
        """Falsify the declared architecture. Weights are untouched.

        `vec` is carried through as well as `weights`. It was omitted when this
        was written, because at the time only the Keras server existed and that
        path populates `weights` while leaving `vec` None. Under
        `TorchFederatedServer` the reverse is true, so dropping `vec` silently
        returned an update with no payload at all, and the shape check downstream
        failed on a None rather than on a mismatch.
        """
        if update.client_id not in self.malicious_clients:
            return update
        metadata = dict(update.metadata)
...
class CompositeAttack(Attack):
    ...
    def shape_update(self, update, context: dict):
        # Without this the base-class no-op silently swallowed every payload
        # composed onto a placement attack: the config recorded `payload=boost`
        # and nothing was boosted.
        for attack in self.attacks:
            update = attack.shape_update(update, context)
        return update

    def targets(self, client_id: int) -> bool:
        # The base implementation reads `self.malicious_clients`, which a
        # composite does not have, so it silently reported every client as
        # honest. Ground truth for the analysis only, never for control flow.
        return any(attack.targets(client_id) for attack in self.attacks)
