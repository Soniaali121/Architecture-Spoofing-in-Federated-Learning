...
def _factor_curves(df, factor):
    """(values, attacked mean, control mean) for one measurement, per factor level."""
    import numpy as np

    atk = df[~df["is_control"].astype(bool)]
    ctl = df[df["is_control"].astype(bool)]
    xs = sorted(df[factor].dropna().unique())
    return xs, atk, ctl


def plot_sweep_frontier(df, factor, path, title=None, xlabel=None, logx=False):
    """One factor: does the attack hold, and does the mechanism hold with it?

    Two panels because infiltration and resemblance answer different questions
    and a single axis would imply they are comparable. Placement can stay pinned
    at 1.000 while resemblance decays underneath it -- which is exactly what the
    epochs sweep shows -- and a figure that plotted only the outcome would hide
    the mechanism weakening.

    Each point is read against its own control at the same factor level, drawn
    in grey. Where the control is non-zero the environment is misplacing honest
    clients too, and the attack should be credited only with the difference.
    """
    import numpy as np

    xs, atk, ctl = _factor_curves(df, factor)
    fig, axes = plt.subplots(1, 2, figsize=(10.5, 3.9))

    ai = atk.groupby(factor)["infiltration"].mean()
    ci = ctl.groupby(factor)["infiltration"].mean()
    axes[0].plot(xs, [ai.get(x, np.nan) for x in xs], "o-", color=SERIES[0],
                 markersize=8, label="attacked", zorder=3)
    axes[0].plot(xs, [ci.get(x, np.nan) for x in xs], "s--", color=_CONTROL_GREY,
                 markersize=7, label="honest control", zorder=2)
    axes[0].set_ylabel("infiltration rate")
    axes[0].set_title("Placement")
    axes[0].set_ylim(-0.05, 1.12)
    axes[0].legend(fontsize=8)

    ar = atk.groupby(factor)["target_resemblance"].mean()
    cr = ctl.groupby(factor)["target_resemblance"].mean()
    axes[1].plot(xs, [ar.get(x, np.nan) for x in xs], "o-", color=SERIES[1],
                 markersize=8, label="attacked", zorder=3)
    axes[1].plot(xs, [cr.get(x, np.nan) for x in xs], "s--", color=_CONTROL_GREY,
                 markersize=7, label="honest control", zorder=2)
    axes[1].axhline(0, color=GRID, lw=1.2)
    axes[1].set_ylabel("resemblance to the target group")
    axes[1].set_title("Mechanism")
    ...
    fig.suptitle(title or f"Sweeping {factor}")
    fig.tight_layout()
    _save(path)


def plot_campaign_grid(frames, path, title=None):
    """Every swept factor in one figure: where does the attack break?

    `frames` maps a display label to (dataframe, factor). One panel each,
    infiltration only, so several independent sweeps can be read at a glance and
    compared for shape rather than for value.

    The panels that are FLAT are as much of the result as the ones that fall.
    The notebook currently passes FOUR frames (epochs, max_train, concentration,
    overlap): two flat and two with a frontier. Attacker count and federation
    length are the other two flat factors, but they are reported in their own
    cell rather than here, so do not describe this figure as showing six.
    """
    import numpy as np
    ...
    for ax, (label, (df, factor)) in zip(axes.flat, frames.items()):
        xs, atk, ctl = _factor_curves(df, factor)
        ai = atk.groupby(factor)["infiltration"].mean()
        ci = ctl.groupby(factor)["infiltration"].mean()
        broke = any(ai.get(x, 1.0) < 0.5 for x in xs)
        ax.plot(range(len(xs)), [ai.get(x, np.nan) for x in xs], "o-",
                color=SERIES[3] if broke else SERIES[0], markersize=7, zorder=3)
        ax.plot(range(len(xs)), [ci.get(x, np.nan) for x in xs], "s--",
                color=_CONTROL_GREY, markersize=6, zorder=2)
        ax.set_xticks(range(len(xs)))
        ax.set_xticklabels([f"{x:g}" for x in xs], fontsize=8)
        ax.set_ylim(-0.05, 1.12)
        ax.set_title(f"{label}{'  (BREAKS)' if broke else ''}", fontsize=10)
        ax.grid(True, axis="y", alpha=0.25)

    for ax in axes.flat[n:]:
        ax.axis("off")
    axes[0][0].set_ylabel("infiltration rate")

    fig.suptitle(title or "Where the attack holds, and where it breaks "
                          "(grey = honest control)")
    fig.tight_layout()
    _save(path)


def campaign_summary(frames):
    """One row per (factor, value): attacked, control, gap, verdict.

    GENERATED from the data rather than typed, so the table cannot drift from
    the numbers it describes. Uses `compare_to_baseline`, which is
    direction-agnostic and reports overlapping intervals as "not
    distinguishable" rather than as a small effect.
    """
    import numpy as np
    import pandas as pd
    ...
    rows = []
    for label, (df, factor) in frames.items():
        atk = df[~df["is_control"].astype(bool)]
        ctl = df[df["is_control"].astype(bool)]
        for x in sorted(df[factor].dropna().unique()):
            a = atk[atk[factor] == x]["infiltration"].dropna()
            c = ctl[ctl[factor] == x]["infiltration"].dropna()
            if not len(a) or not len(c):
                continue
            v = compare_to_baseline(
                "infiltration", float(a.mean()), baseline=float(c.mean()),
                interval=boot_ci(a)[1:], baseline_interval=boot_ci(c)[1:])
            res = atk[atk[factor] == x]["target_resemblance"].mean()
            rows.append({"experiment": label, "factor": factor, "value": x,
                         "attacked": round(float(a.mean()), 3),
                         "control": round(float(c.mean()), 3),
                         "gap": round(v.get("gap", np.nan), 3),
                         "resemblance": round(float(res), 3),
                         "verdict": v["verdict"]})
    return pd.DataFrame(rows)
