...
class TorchFederatedServer:
    ...
    def __init__(self, input_shape, num_classes, clusterer, arch: str = "mlp_flat",
                 ...
                 client_archs: Optional[List[str]] = None):
        ...
        # ARCHITECTURE MODE. Passing `client_archs` switches the server from the
        # distribution study's single-architecture assumption to one model per
        # architecture, with cluster ids that ARE architecture names. This is the
        # torch counterpart of what `FederatedServer` has always done, added so
        # the architecture channel can use `sweeps.py`, the guard, the 51-column
        # schema and the GPU rather than the CPU-only TensorFlow loop.
        #
        # Everything below is written so that `client_archs=None` takes exactly
        # the code path it took before, because every Part A number depends on it.
        self.client_archs = list(client_archs) if client_archs else None
        self.arch_mode = self.client_archs is not None
        if self.arch_mode:
            self.arch_pool = sorted(set(self.client_archs))
        ...
        if self.arch_mode:
            # One model per architecture, keyed by the architecture NAME. Each is
            # a different length, which is exactly why cross-cluster similarity
            # is never computed here: `ArchitectureClusterer` groups on the
            # declared field and sets `needs_delta = False`.
            for a in self.arch_pool:
                torch.manual_seed(seed * 1000 + self.arch_pool.index(a))
                self.cluster_models[a] = flatten(
                    build_torch_model(a, input_shape, num_classes).state_dict())
        elif init_mode == "shared":
        ...
        # Head scoping is a distribution-study concern. In architecture mode each
        # architecture has its own head at its own offsets, so a single index
        # array would be wrong for all but one of them, and the architecture
        # clusterer does not use similarity at all.
        if self.arch_mode and getattr(clusterer, "scope", None) == "head":
            raise ValueError(
                "head scoping is meaningless in architecture mode: each "
                "architecture has a differently shaped head, and "
                "ArchitectureClusterer groups on the declared field rather than "
                "on update similarity.")
        ...
        # Audit trail for the free defence, so a naive spoofer is recorded as
        # caught rather than silently vanishing from the detection statistics.
        self.shape_rejections: List[tuple] = []
    ...
    # -- placement ---------------------------------------------------------- #
    def _arch_of(self, cluster_id: str) -> str:
        """Which architecture a cluster's model is.

        In architecture mode the cluster id IS the architecture name, so every
        cluster holds a differently shaped model and `self.arch` is wrong for all
        but one of them. Loading a 578,894-entry vector into a 110,156-entry
        template is how that surfaces, which is exactly what it did.
        """
        if self.arch_mode and cluster_id in self.cluster_models:
            return cluster_id
        return self.arch
    ...
    def _ifca_choice(self, client_id: int, X, y) -> int:
        ...
        for cid in sorted(self.cluster_models):
            loss, _ = evaluate_vec(self.cluster_models[cid], self._arch_of(cid),
    ...
    def run_round(self, client_data, round_num: int = 0, epochs: int = 2,
        ...
        for client_id, (X, y) in enumerate(client_data):
            if self.arch_mode:
                # THE ARCHITECTURE ATTACK. Same contract as `FederatedServer`,
                # so `ArchSpoof` is reused unchanged:
                #
                #   training_arch()  returns the architecture to TRAIN, and only
                #                    an ADAPTIVE spoofer gets one. A naive spoofer
                #                    gets None and trains its own.
                #   apply()          overwrites metadata['arch'], the DECLARED
                #                    field, for every malicious client either way.
                #
                # So adaptive means trained == declared and the update is
                # shape-valid for the victim cluster; naive means they differ and
                # the shape check rejects it downstream for free.
                true_arch = self.client_archs[client_id]
                train_arch = self.attack.training_arch(client_id, true_arch) or true_arch
                # Warm-start from the model of the architecture actually being
                # trained. Starting from any other is a shape error, not an attack.
                home_cid = train_arch
                global_vec = self.cluster_models[train_arch]
            else:
                true_arch, train_arch = self.arch, self.arch
            ...
            update = train_client_torch(
                client_id, X_train, y_train, train_arch, global_vec,
                self.input_shape, self.num_classes, epochs=epochs,
                batch_size=batch_size, seed=self.seed * 1000 + round_num * 100 + client_id,
                true_arch=true_arch)

            if self.arch_mode:
                # Lets the attacker falsify the declared field. For an honest
                # client this is a no-op, and metadata['arch'] stays as the
                # architecture that was trained.
                update = self.attack.apply(update)
                declared = update.metadata.get("arch", true_arch)
                update.metadata["true_arch"] = true_arch
                update.metadata["trained_arch"] = train_arch
                # The free defence: does the submitted vector actually fit the
                # model of the cluster it is asking to join?
                update.metadata["shape_matches_declared"] = bool(
                    declared in self.cluster_models
                    and len(update.vec) == len(self.cluster_models[declared]))
        ...
        for cid, members in clusters.items():
            if self.arch_mode:
                # THE SHAPE CHECK, and it is free. An update that does not fit
                # this cluster's model cannot be averaged into it, so a naive
                # spoofer is stopped here without any defence being involved.
                #
                # Tagged "shape" rather than "fingerprint" deliberately: it is
                # not the fingerprint verifier's catch, and pooling the two is
                # how a defence gets credited with work the shape check did. An
                # ADAPTIVE spoofer passes this by construction, which is exactly
                # what makes it the interesting case.
                want = len(self.cluster_models[cid]) if cid in self.cluster_models else None
                kept = []
                for u in members:
                    if want is not None and len(u.vec) != want:
                        selected[u.client_id] = False
                        u.metadata["shape_rejected"] = True
                        self.shape_rejections.append(
                            (round_num, u.client_id, cid,
                             bool(u.metadata.get("is_attacker"))))
                    else:
                        u.metadata["shape_rejected"] = False
                        kept.append(u)
                if not kept:
                    # Every member rejected. Keep the cluster's model as it was
                    # rather than dropping the key, which would silently shrink
                    # the federation mid-run.
                    new_models[cid] = self.cluster_models[cid]
                    continue
                members = kept
    ...
    def evaluate(self, X_test, y_test, verbose: bool = False,
        ...
        for cid, vec in self.cluster_models.items():
            ...
            l, a = evaluate_vec(vec, self._arch_of(cid), self.input_shape,
    ...
    def fit(self, client_data, X_test, y_test, rounds: int = 3, epochs: int = 2,
        ...
        for r in range(rounds):
            ...
            # Pairwise cosine over every client's delta, for observability only.
            #
            # It cannot exist in architecture mode: clients run differently
            # shaped models, so the deltas have different lengths and there is no
            # space to compare them in. That is not a limitation to work around,
            # it is the reason `ArchitectureClusterer` groups on the declared
            # field and sets `needs_delta = False`. Recorded as absent rather
            # than faked, so nothing downstream can quietly treat a missing
            # similarity matrix as a computed one.
            if self.arch_mode:
                D, S, client_ids = None, None, [u.client_id for u in updates]
            else:
